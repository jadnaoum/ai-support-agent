...
| turn_state | JSONB | Nullable. Persists actions_taken, consecutive_blocks, last_clarification_source across turns. Cleared on escalation. |
...
class AgentState(TypedDict):
	...
	actions_taken: list      # Audit trail of all service calls. Persisted across turns via turn_state. Used by confirmation gate to check prior confirmation_required entries.
	last_clarification_source: str   # Persisted across turns via turn_state. Source of the last clarifying question asked: "intent" or "emotion" or "" (none). If non-empty and intent is still needs_clarification, escalate with unable_to_clarify instead of asking again.
    context_summary: str     # Escalation context summary. Built by handle_escalation, surfaced to eval judge via TestChatResponse.
	service_call_count: int  # Number of service calls made this turn. Set to 1 when the first service dispatch happens in Pass 1 (not reset to 0 at turn start). Graph loop checks service_call_count < SERVICE_CALL_LIMIT in Pass 2, so the effective maximum is SERVICE_CALL_LIMIT - 1 additional calls after the first. Default SERVICE_CALL_LIMIT: 3.
	consecutive_blocks: int  # Persisted across turns via turn_state. Counts consecutive input-guard blocks. At 3, escalates with reason repeated_blocks. Resets to 0 on any unblocked turn. Cleared on escalation.
```

### Graph nodes
1. **conversation_agent** — The only customer-facing agent. Reads the latest customer message, decides what it needs (KB lookup, action execution, escalation, or clarification), calls the appropriate service(s), and generates the final customer-facing response. All intent classification happens here — no separate supervisor. Intent classification receives the last 5 turns of conversation history (~10 messages filtered to customer/agent roles), enabling follow-up resolution (bare order numbers, confirmations, reasons in context). Uses five intents: `knowledge_query`, `action_request`, `escalation_request`, `needs_clarification`, and `general`. When intent is `needs_clarification`, generates a targeted clarifying question (max 1 per turn, never two in a row). If the previous turn was already a clarification and intent is still `needs_clarification`, escalates with reason `unable_to_clarify` instead of asking again. When escalating, calls the escalation handler inline and sets the handoff response directly — escalation is not a separate graph node. **Tone:** defined by a persona-based `response_prompt` in `prompts/production.yaml` — direct, action-oriented, no performative empathy, no emoji, plain text output. Tone is shaped by persona description and example exchanges, not a rulebook. Situational warmth: routine issues get efficient tone; high-impact issues (damaged items, urgent need) get brief warmer acknowledgment. On blocked inputs (off_topic or prompt_injection, blocks 1-2), generates a redirect message via an additional LLM call using `redirect_prompt` — this is a separate LLM call from the intent classifier and response generator.
...
→ deterministic heuristic first: `_service_needs_loop(state)` checks whether the last service result warrants a follow-up (e.g. `success: False` on an action result). If the heuristic returns False (clean success), skip the LLM call and go straight to response generation.
→ heuristic returns True AND service_call_count < SERVICE_CALL_LIMIT → LLM call (`loop_decision_prompt`) decides: another service call → route to next service, or respond → compose final response
→ service_call_count >= SERVICE_CALL_LIMIT → compose final response → END
...
- **Failure mode**: fails open — if the LLM call or JSON parse raises an exception, the message is treated as `safe` and passed through. This is intentional: a guard outage should not block all customer messages. The output guard provides a second safety net downstream.
...
- **`mock_account_state`** — Feeds the mock tool layer. When present, action service tools return responses based on this mock data instead of querying the database. The agent accesses order data through tool calls — same path as production. Customer name is injected into `customer_context` if provided. Turn state persistence: actions_taken, consecutive_blocks, and last_clarification_source are persisted across turns via a `turn_state` JSONB column on the conversations table. After each graph run, these fields are saved. At the start of the next turn, they are loaded and merged into initial_state. Saved in the same db.commit() as the agent message. Cleared to None on escalation.
